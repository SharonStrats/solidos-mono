...
exports.FedcmHttpHandler = void 0;
 ...
 */
class FedcmHttpHandler extends community_server_1.HttpHandler {
    constructor(baseUrl, cookieStore, webIdStore) {
        super();
        this.logger = (0, community_server_2.getLoggerFor)(this);
    ...
    async get_token(id, secret, dpopHeader) {
    ...
    async get_client_id_secret(authorization, webId) {
    ...
    async deleteToken(tokenId, authorization) {
        const indexResponse = await fetch(`${this.baseUrl}.account/`, {
            headers: { authorization: `CSS-Account-Token ${authorization}` }
        });
        let { controls } = await indexResponse.json();
        ...
        const listOfTokensJson = await listOfTokensResp.json();
    ...
    async handleWebIdentity({ request, response }) {
        // 3.1
    ...
    async handleFedcmJSON({ request, response }) {
        // 3.2
        //
        const config = {
    ...
    async handleAccountsEnpoint({ request, response }) {
        // 3.3
    ...
    async handleClientMetadataEndpoint({ request, response }) {
        // 3.4
    ...
    async handleToken({ request, response }) {
        ...
        const r = await (0, community_server_3.readableToString)(request);
        ...
        const { tokenId, secret } = await this.get_client_id_secret(cssAccountCookie, webId);
        const { access_token: accessToken } = await this.get_token(tokenId, secret, dpopHeader);
    ...
    async handleDisconnect({ request, response }) {
        // 3.6
...
exports.FedcmHttpHandler = FedcmHttpHandler;
//# sourceMappingURL=FedcmHttpHandler.js.map